...
import { ArrowLeft, Lock, Mail, Phone, Stethoscope, UserRound } from "lucide-react";
...
export default function DoctorSignIn() {
  ...
  const handleSignup = async () => {
    ...
    setOtpMessage(
      result.otp?.dev_otp
        ? `Local testing OTP: ${result.otp.dev_otp}`
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-[#020617] via-[#071133] to-[#0F172A] px-6">
      <div className="w-full max-w-md rounded-[32px] border border-white/10 bg-white/10 p-10 shadow-2xl backdrop-blur-xl">
        ...
        <h1 className="text-center text-4xl font-bold text-white">Doctor Signup</h1>
        <p className="mt-3 text-center text-slate-400">Create a doctor account for the clinical dashboard.</p>
        ...
        <div className="mt-8 space-y-5">
          ...
          <div className="flex items-center gap-3 rounded-2xl bg-slate-900/70 px-4 py-4">
            ...
            <input
              ...
              disabled={verificationStarted}
              placeholder="Phone for OTP login"
              className="w-full bg-transparent text-white outline-none placeholder:text-slate-500"
            />
          </div>
          ...
          <Link to="/doctor-login" className="block text-center text-sm text-slate-400">
